...
rootClustersAgg = tk.Tk()
rootClustersAgg.title("Graphique t-SNE Cluster DBSCAN")
seaborn.set_style("white")

figClusterAgg = plt.figure()
axClusterAgg = figClusterAgg.add_subplot(111, projection='3d')
...
# =============================================================================
# Variables pour AgglomerativeClustering
# =============================================================================

nb_clusters = 5

linkage_method = 'ward'  # Méthode de regroupement ('ward', 'complete', 'average', 'single')

# =============================================================================
# Changement de méthode de clustering : AgglomerativeClustering
# =============================================================================

# Initialisation de l'algorithme
agg_clustering = AgglomerativeClustering(n_clusters=nb_clusters, linkage=linkage_method)

# Appliquer le clustering aux données à 3 dimensions
data['Cluster'] = agg_clustering.fit_predict(data_3dim)

# Ajouter les informations du cluster pour chaque point
cluster_data = []  # Réinitialiser pour AgglomerativeClustering
...
axClusterAgg.set_xlabel("Dimension 1")
axClusterAgg.set_ylabel("Dimension 2")
axClusterAgg.set_zlabel("Dimension 3")
axClusterAgg.set_title("Plan factoriel")

# Ajouter une légende dynamique
axClusterAgg.legend(loc='center left', bbox_to_anchor=(1, 0.5), title="Légende")

figClusterAgg.savefig("/Users/camilleauvity/Desktop/ECOLE/Projet-Industriel/resultats t-SNE/Plot-tSNE/Projection cluster k-means 3D.png", format='png', dpi=300)  # `dpi=300` pour une haute qualité

# Intégrer le graphique Matplotlib dans la fenêtre Tkinter
canvasCluster = FigureCanvasTkAgg(figClusterAgg, master=rootClustersAgg)
canvasCluster.draw()
canvasCluster.get_tk_widget().pack(expand=True, fill="both")

# Ajouter une barre d'outils de navigation (avec zoom et pan)
toolbarCluster = NavigationToolbar2Tk(canvasCluster, rootClustersAgg)
toolbarCluster.update()
toolbarCluster.pack(side=tk.BOTTOM, fill=tk.X)

# Lancer l'application Tkinter
rootClustersAgg.mainloop()
