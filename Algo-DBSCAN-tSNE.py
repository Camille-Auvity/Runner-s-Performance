...
rootClustersDBSCAN = tk.Tk()
rootClustersDBSCAN.title("Graphique t-SNE Cluster DBSCAN")
seaborn.set_style("white")

figClusterDBSCAN = plt.figure()
axClusterDBSCAN = figClusterDBSCAN.add_subplot(111, projection='3d')
...
eps = 1.6  # Rayon maximal pour considérer les voisins
min_samples = 7  # Nombre minimal de points pour former un cluster

# =============================================================================
# Changement de méthode de clustering
# =============================================================================

# Appliquer DBSCAN sur les données à 3 dimensions
dbscan = DBSCAN(eps=eps, min_samples=min_samples)
data['Cluster'] = dbscan.fit_predict(data_3dim)

# Ajouter les informations du cluster pour chaque point
cluster_data = []  # Réinitialiser pour DBSCAN
...
clusters = [[] for _ in range(len(set(data['Cluster'])) - (1 if -1 in data['Cluster'] else 0))]
for idx, row in df_clusters.iterrows():
    cluster_index = int(row['Cluster'])
    if cluster_index != -1:  # Exclure les points considérés comme "bruit"
        clusters[cluster_index].append([row['Dim1'], row['Dim2'], row['Dim3']])
clusters = clusters[:-1]

# Calculer les centroids pour les clusters valides
centroids = []
...
axClusterDBSCAN.set_xlabel("Dimension 1")
axClusterDBSCAN.set_ylabel("Dimension 2")
axClusterDBSCAN.set_zlabel("Dimension 3")
axClusterDBSCAN.set_title("Plan factoriel")

# Ajouter une légende dynamique
axClusterDBSCAN.legend(loc='center left', bbox_to_anchor=(1, 0.5), title="Légende")

figClusterDBSCAN.savefig("/Users/camilleauvity/Desktop/ECOLE/Projet-Industriel/resultats t-SNE/Plot-tSNE/Projection cluster k-means 3D.png", format='png', dpi=300)  # `dpi=300` pour une haute qualité

# Intégrer le graphique Matplotlib dans la fenêtre Tkinter
canvasCluster = FigureCanvasTkAgg(figClusterDBSCAN, master=rootClustersDBSCAN)
canvasCluster.draw()
canvasCluster.get_tk_widget().pack(expand=True, fill="both")

# Ajouter une barre d'outils de navigation (avec zoom et pan)
toolbarCluster = NavigationToolbar2Tk(canvasCluster, rootClustersDBSCAN)
toolbarCluster.update()
toolbarCluster.pack(side=tk.BOTTOM, fill=tk.X)

# Lancer l'application Tkinter
rootClustersDBSCAN.mainloop()
