...
data = pandas.read_csv('/Users/camilleauvity/Desktop/ECOLE/Projet-Industriel/resultats ACP/Data_complet_ap_analyse.csv')

# Sélectionner les dimensions pertinentes pour le clustering (Dim1, Dim2, Dim3)
data_3dim = data[["Dim1", "Dim2", "Dim3"]]


rootClusterKmeans = tk.Tk()
rootClusterKmeans.title("Graphique ACP Cluster k-means")
seaborn.set_style("white")

figClusterKmeans = plt.figure()
axClusterKmeans = figClusterKmeans.add_subplot(111, projection='3d')
...
data['Cluster'] = kmeans.fit_predict(data_3dim)
...
for idx, row in df_clusters.iterrows():
    cluster_index = int(row['Cluster'])  # Indice du cluster (0 à 8)
    clusters[cluster_index].append([row['Dim1'], row['Dim2'], row['Dim3']])  # Ajouter les coordonnées du point

# Calculer le centroid pour chaque cluster
for i in range(9):  # Nous avons 9 clusters
    cluster_points = df_clusters[df_clusters['Cluster'] == i][['Dim1', 'Dim2', 'Dim3']].values
    centroid = cluster_points.mean(axis=0)  # Calculer la moyenne des coordonnées pour chaque cluster
...
for j, cluster in enumerate(clusters):
    ...
    # Affichage des points du cluster avec la couleur associée
    axClusterKmeans.scatter(
...
axClusterKmeans.scatter(
    ...
    label='Centre Cluster', color=colors[18], s=100, marker='o')
    
for i in range(len(centroids)):
    for j in range(i + 1, len(centroids)):
        axClusterKmeans.plot([centroids[i, 0], centroids[j, 0]], 
                [centroids[i, 1], centroids[j, 1]], 
                [centroids[i, 2], centroids[j, 2]], color='gray', linewidth=1)

#axClusterKmeans.plot_trisurf(centroids[:, 0], centroids[:, 1], centroids[:, 2], color='cyan', alpha=0.3)

# Ajouter des labels, un titre et une légende
axClusterKmeans.set_xlabel("Dimension 1")
axClusterKmeans.set_ylabel("Dimension 2")
axClusterKmeans.set_zlabel("Dimension 3")
axClusterKmeans.set_title("Plan factoriel")

# Ajouter une légende dynamique
axClusterKmeans.legend(loc='center left', bbox_to_anchor=(1, 0.5), title="Légende")

figClusterKmeans.savefig("/Users/camilleauvity/Desktop/ECOLE/Projet-Industriel/resultats ACP/Plot-ACP/Projection cluster k-means 3D.png", format='png', dpi=300)  # `dpi=300` pour une haute qualité

# Intégrer le graphique Matplotlib dans la fenêtre Tkinter
canvasCluster = FigureCanvasTkAgg(figClusterKmeans, master=rootClusterKmeans)
canvasCluster.draw()
canvasCluster.get_tk_widget().pack(expand=True, fill="both")

# Ajouter une barre d'outils de navigation (avec zoom et pan)
toolbarCluster = NavigationToolbar2Tk(canvasCluster, rootClusterKmeans)
toolbarCluster.update()
toolbarCluster.pack(side=tk.BOTTOM, fill=tk.X)

# Lancer l'application Tkinter
rootClusterKmeans.mainloop()
...
pivot_table.to_csv('/Users/camilleauvity/Desktop/ECOLE/Projet-Industriel/pivot_tableACP.csv', index=False)
...
plt.tight_layout()
plt.savefig("/Users/camilleauvity/Desktop/ECOLE/Projet-Industriel/resultats ACP/Plot-ACP/Analyser le clustering - Barres empilées.png", format='png', dpi=300)  # `dpi=300` pour une haute qualité
plt.show()
...
plt.tight_layout()
plt.savefig("/Users/camilleauvity/Desktop/ECOLE/Projet-Industriel/resultats ACP/Plot-ACP/Analyser le clustering - Diagrammes à Barres.png", format='png', dpi=300)  # `dpi=300` pour une haute qualité
plt.show()
...
plt.tight_layout()
plt.savefig("/Users/camilleauvity/Desktop/ECOLE/Projet-Industriel/resultats ACP/Plot-ACP/Analyser le clustering - lignes connectées.png", format='png', dpi=300)  # `dpi=300` pour une haute qualité
plt.show()
...
plt.ylabel("Noms")
plt.tight_layout()
plt.savefig("/Users/camilleauvity/Desktop/ECOLE/Projet-Industriel/resultats ACP/Plot-ACP/Analyser le clustering - Headmap des données.png", format='png', dpi=300)  # `dpi=300` pour une haute qualité
plt.show()
...
output_path = '/Users/camilleauvity/Desktop/ECOLE/Projet-Industriel/clusters_with_points_sklearnACP.csv'
